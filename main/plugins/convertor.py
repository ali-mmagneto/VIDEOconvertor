...
async def mp3(event, msg):
    ...
    try:
        ...
        print(e)
        return await edit.edit(f"İndirme Sırasında Bir Hata Oluştu!\n\nİletişim [Grup]({SUPPORT_LINK})")
    try:
        await edit.edit("Dönüştürülüyor.")
        bash(f"ffmpeg -i {name} -codec:a libmp3lame -q:a 0 {out}.mp3")
    except Exception as e:
        os.rmdir("audioconvert")
        print(e)
        return await edit.edit(f"Dönüştürme Sırasında Bir Hata Oluştu!\n\nİletişim [Grup]({SUPPORT_LINK})")
    try:
        UT = time.time()
        uploader = await fast_upload(f'{out}.mp3', f'{out}.mp3', UT, Drone, edit, '**Ykleniyor:**')
        await Drone.send_file(event.chat_id, uploader, thumb=JPG, caption=f'**Sesi Çıkaran** : @{BOT_UN}', force_document=True)
    except Exception as e:
        os.rmdir("audioconvert")
        print(e)
...
async def flac(event, msg):
    ...
    try:
        UT = time.time()
        uploader = await fast_upload(f'{out}.flac', f'{out}.flac', UT, Drone, edit, '**Yükleniyor:**')
        await Drone.send_file(event.chat_id, uploader, thumb=JPG, caption=f'**Sesi Çıkaran** : @{BOT_UN}', force_document=True)
    except Exception as e:
        os.rmdir("audioconvert")
        print(e)
...
async def wav(event, msg):
    ...
    try:
        UT = time.time()
        uploader = await fast_upload(f'{out}.wav', f'{out}.wav', UT, Drone, edit, '**Yükleniyor:**')
        await Drone.send_file(event.chat_id, uploader, thumb=JPG, caption=f'**Sesi Çıkaran** : @{BOT_UN}', force_document=True)
    except Exception as e:
        os.rmdir("audioconvert")
        print(e)
...
async def mp4(event, msg):
    ...
    try:
        UT = time.time()
        uploader = await fast_upload(f'{out}.mp4', f'{out}.mp4', UT, Drone, edit, '**Yükleniyor:**')
        await Drone.send_file(event.chat_id, uploader, thumb=JPG, caption=f'**Dönüştüren** : @{BOT_UN}', force_document=True)
    except Exception as e:
        print(e)
        return await edit.edit(f"Yükleme Sırasında Bir Hata Oluştu!\n\nİletişim [Grup]({SUPPORT_LINK})")
    await edit.delete()                      
...
async def file(event, msg):
    ...
    try:
        UT = time.time()
        uploader = await fast_upload(f'{name}', f'{name}', UT, Drone, edit, '**Yükleniyor:**')
        await Drone.send_file(event.chat_id, uploader, thumb=JPG, caption=f'**Dönüştüren** : @{BOT_UN}', force_document=True)
    except Exception as e:
        print(e)
        return await edit.edit(f"Yükleme Sırasında Bir Hata Oluştu!\n\nİletişim [Grup]({SUPPORT_LINK})")
...
async def video(event, msg):
    ...
    try:
        ...
        UT = time.time()
        uploader = await fast_upload(f'{out}', f'{out}', UT, Drone, edit, '**Yükleniyor:**')
        await Drone.send_file(event.chat_id, uploader, thumb=JPG2, caption=f'**Dönüştüren** : @{BOT_UN}', attributes=attributes, force_document=False)
    except Exception as e:
        print(e)
        return await edit.edit(f"Yükleme Sırasında Bir Hata Oluştu!\n\nİletişim [Grup]({SUPPORT_LINK})")
    await edit.delete()
